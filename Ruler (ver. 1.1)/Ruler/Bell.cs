...
namespace Ruler
{
    ...
    public class Bell : ABell, IBell, ICloneable
    {
        ...
        private List<List<int>> aBell;     // Начальный список чисел Белла (наборы длин)
        private List<List<int>> aBellFull; // Полный список всех перестановок набор длин
        private int[] aBellTemp;   // Временный массив для построения набора длин
        ...
        void CopyTemp()
        {
            aBell.Add(new List<int>());
            for (int i = (aBellTemp.Length - 1); i >= 0; i--)
                if (aBellTemp[i] != 0)
                    aBell[aBell.Count - 1].Add(aBellTemp[i]);
